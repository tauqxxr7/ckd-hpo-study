...
On the UCI CKD dataset, the best summarized result was obtained using SVM with Random Search, reaching 99.33\% $\pm$ 0.36\% F1-score, with 99.39\% precision and 99.27\% recall. XGBoost with Random Search reached 99.08\% F1-score, while Random Forest with Random Search reached 98.95\% F1-score. The UCI results therefore show consistently high performance across model families, but the small benchmark size means that these estimates should be interpreted as validation-set performance within the experimental protocol rather than evidence of deployment readiness.

On the eICU-derived dataset, Random Search produced the strongest summarized configuration, with Random Forest reaching 94.54\% $\pm$ 0.42\% F1-score, 94.90\% precision, and 94.18\% recall. XGBoost with Random Search was close at 94.36\% F1-score, while SVM with Random Search reached 93.74\%. Grid Search was lower for each model family on the eICU summary table, including 92.31\% for Random Forest, 92.88\% for XGBoost, and 91.96\% for SVM. This pattern suggests that exhaustive fixed-grid evaluation was less efficient under the larger-dataset budget considered here.
...
\caption{F1-score comparison across datasets, model families, and hyperparameter optimization strategies.}
...
\caption{Runtime comparison for hyperparameter optimization strategies on the UCI and eICU datasets.}
...
\caption{Best summarized F1-score versus runtime for each dataset and hyperparameter optimization method.}
...
\caption{Mean F1-score comparison of Grid Search, Random Search, Bayesian Optimization/TPE, CMA-ES, and Hyperband across model families.}
