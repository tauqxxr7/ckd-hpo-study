...
The paper avoids claims of clinical deployment readiness. Instead, it presents a comparative experimental analysis intended to support reproducible model development and more transparent reporting of optimization choices.
...
\begin{figure}[t]
\centering
\fbox{\parbox{0.85\linewidth}{\centering Placeholder for workflow diagram.}}
\caption{Overall workflow for preprocessing, hyperparameter optimization, model training, and evaluation.}
\label{fig:workflow}
\end{figure}
...
On the UCI CKD dataset, the best summarized result was obtained using SVM with Random Search, reaching 99.33\% $\pm$ 0.36\% F1-score. Random Forest and XGBoost also achieved high F1-scores above 98\% under several HPO methods. These results are consistent with the relatively structured nature of the UCI benchmark, but they should be interpreted cautiously because the dataset is small.

On the eICU-derived dataset, Random Search achieved the best summarized F1-score of 94.54\%. Grid Search performed slightly worse and required substantially more runtime. This suggests that exhaustive search over a fixed grid may be inefficient when the dataset is larger and the computational budget is constrained.

\begin{figure}[t]
\centering
\fbox{\parbox{0.85\linewidth}{\centering Placeholder for ROC curve.}}
\caption{Receiver operating characteristic curve for the best-performing CKD prediction configuration.}
\label{fig:roc}
\end{figure}

\begin{figure}[t]
\centering
\fbox{\parbox{0.85\linewidth}{\centering Placeholder for confusion matrix.}}
\caption{Confusion matrix for the selected model configuration on the held-out evaluation split.}
\label{fig:confusion}
\end{figure}

\begin{figure}[t]
\centering
\fbox{\parbox{0.85\linewidth}{\centering Placeholder for F1-score versus runtime comparison.}}
\caption{Comparison of F1-score and runtime across hyperparameter optimization strategies.}
\label{fig:f1_runtime}
\end{figure}

\begin{figure}[t]
\centering
\fbox{\parbox{0.85\linewidth}{\centering Placeholder for HPO method comparison chart.}}
\caption{Performance comparison of Grid Search, Random Search, Bayesian Optimization/TPE, CMA-ES, and Hyperband.}
...
This study compared five HPO methods for CKD prediction across a small public benchmark and a larger critical-care dataset. Random Search produced the best reported UCI configuration when paired with SVM and achieved the strongest summarized eICU result, while also providing the most practical time-performance tradeoff. Grid Search remained easy to interpret but became inefficient and less effective on the larger dataset under the evaluated budget. The results emphasize that HPO strategy and runtime should be reported transparently in clinical machine learning studies.
